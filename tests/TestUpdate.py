...
import laqueli
from laqueli import logger
from laqueli.Update import check_updates, get_ip_for_url, update_winapp2, user_agent
import laqueli.Update
...
class UpdateTestCase(common.BleachbitTestCase):
    ...
    def test_UpdateCheck_fake(self):
        ...
        # fake network
        original_open = laqueli.Update.build_opener
        xml = ""
    ...
    def test_UpdateCheck_real(self):
        ...
        preserve_url = laqueli.update_check_url
        for url in ('http://localhost/doesnotexist', 'https://httpstat.us/500'):
            laqueli.update_check_url = url
            self.assertEqual(
                check_updates(True, False, None, None),
                ())
        laqueli.update_check_url = preserve_url

    def test_get_ip_for_url(self):
        """Unit test for get_ip_for_url()"""
        for good_url in ('https://www.example.com', laqueli.update_check_url):
    ...
    def test_update_url(self):
        ...
        from laqueli.Update import build_opener
        opener = build_opener()
        opener.addheaders = [('accept','text/*')]
        import urllib
        try:
            handle = opener.open(laqueli.update_check_url)
        except urllib.error.HTTPError as e:
            logger.exception('HTTP error, url: %s\nheaders:\n%s', laqueli.update_check_url, e.headers)
            raise e
        doc = handle.read()
        import xml
    ...
    def test_update_winapp2(self):
        from laqueli import personal_cleaners_dir
