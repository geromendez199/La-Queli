...
import laqueli.Options
from laqueli import NoOptionError
...
class OptionsTestCase(common.BleachbitTestCase):
    ...
    def test_Options(self):
        ...
        for bkey in laqueli.Options.boolean_keys:
    ...
    def test_init_configuration(self):
        ...
        if os.path.exists(laqueli.options_file):
            os.remove(laqueli.options_file)
        self.assertNotExists(laqueli.options_file)
        laqueli.Options.init_configuration()
        self.assertExists(laqueli.options_file)

    def test_is_corrupt(self):
        """Test is_corrupt()"""
        def _test_is_corrupt(contents, expect_is_corrupt):
            with open(laqueli.options_file, 'w') as f:
                f.write(contents)
            o = laqueli.Options.Options()
    ...
    def test_percent(self):
        ...
        opt = laqueli.Options.options.config
