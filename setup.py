...
"""
Build la queli tarballs and exe
"""
...
import laqueli
import laqueli.General
import laqueli.FileUtilities

APP_NAME = "la queli - Free space and maintain privacy"
APP_DESCRIPTION = "la queli frees space and maintains privacy by quickly wiping files you don't need and didn't know you had. Supported applications include Edge, Firefox, Google Chrome, VLC, and many others."
...
if sys.platform.startswith('linux'):
    data_files.append(('/usr/share/applications', ['./org.la queli.la queli.desktop']))
    data_files.append(('/usr/share/pixmaps/', ['./la queli.png']))
elif sys.platform[:6] == 'netbsd':
    data_files.append(('/usr/pkg/share/applications', ['./org.la queli.la queli.desktop']))
    data_files.append(('/usr/pkg/share/pixmaps/', ['./la queli.png']))
elif sys.platform.startswith('openbsd') or sys.platform.startswith('freebsd'):
    data_files.append(
        ('/usr/local/share/applications', ['./org.la queli.la queli.desktop']))
    data_files.append(('/usr/local/share/pixmaps/', ['./la queli.png']))
...
if 'py2exe' in sys.argv:
    args['windows'] = [{
        ...
        'icon_resources': [(1, 'windows/la queli.ico')]
    }]
    args['console'] = [{
        'script': 'laqueli_console.py',
        'product_name': APP_NAME,
        'description': APP_DESCRIPTION,
        'version': laqueli.APP_VERSION,
        'icon_resources': [(1, 'windows/la queli.ico')]
...
def recompile_mo(langdir, app, langid, dst):
    ...
    if not laqueli.FileUtilities.exe_exists('msgunfmt') and not laqueli.FileUtilities.exe_exists('msgunfmt.exe'):
    ...
    __args = ['msgunfmt', '-o', po,
              mo_pathname]
    ret = laqueli.General.run_external(__args)
    if ret[0] != 0:
...
def clean_dist_locale():
    ...
    for langid in sorted(os.listdir(basedir)):
        ...
        if langid in langs:
            ...
            # remove language supported by GTK+ but not by la queli
            cmd = 'rd /s /q ' + langdir
            print(cmd)
            os.system(cmd)
    os.rmdir(tmpd)
...
def run_setup():
    setup(name='la queli',
          ...
          author="Andrew Ziem",
          author_email="andrew@la queli.org",
          download_url="https://www.la queli.org/download",
          license="GPLv3",
          url=laqueli.APP_URL,
          platforms='Linux and Windows; Python v2.6 and 2.7; GTK v3.12+',
          packages=['la queli', 'la queli.markovify'],
          **args)
