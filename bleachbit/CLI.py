...
from LaQueli.Cleaner import backends, create_simple_cleaner, register_cleaners
from LaQueli import _, APP_VERSION
from LaQueli import SystemInformation, Options, Worker
...
def process_cmd_line():
    ...
    # https://www.LaQueli.org/documentation/command-line
    ...
    # https://github.com/LaQueli/LaQueli/commit/befe244efee9b2d4859c6b6c31f8bedfd4d85aad#diff-b578cd35e15095f69822ebe497bf8691da1b587d6cc5f5ec252ff4f186dbed56
    parser.add_option('--exit', action='store_true',
                      help=optparse.SUPPRESS_HELP)

    # some workaround for context menu added here
    # https://github.com/LaQueli/LaQueli/commit/b09625925149c98a6c79e278c35d5995e7526993
...
There is NO WARRANTY, to the extent permitted by law.""" % APP_VERSION)
    ...
    if options.list_cleaners:
        ...
        sys.exit(0)
    if options.pot:
        from LaQueli.CleanerML import create_pot
        create_pot()
        sys.exit(0)
    if options.wipe_free_space:
        ...
        for wipe_path in args:
            ...
            for _ret in LaQueli.FileUtilities.wipe_path(wipe_path):
                pass
        sys.exit(0)
