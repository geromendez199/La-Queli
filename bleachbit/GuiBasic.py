...
from LaQueli import _, ModuleNotFoundError
...
def delete_confirmation_dialog(parent, mention_preview, shred_settings=False):
    ...
    if shred_settings:
        notice_text = _("This function deletes all LaQueli settings and then quits the application. Use this to hide your use of LaQueli or to reset its settings. The next time you start LaQueli, the settings will initialize to default values.")
...
def open_url(url, parent_window=None, prompt=True):
    ...
    if prompt:
        ...
        if not ret:
            ...
            host = ret.group(2)
        # TRANSLATORS: %s expands to www.LaQueli.org or similar
