...
the reference
* HKCU\\Software\\LaQueli
* CurrentVersion

These are the terms:
* 'HKCU' is an abbreviation for the hive HKEY_CURRENT_USER.
* 'HKCU\Software\LaQueli' is the key name.
* 'Software' is a sub-key of HCKU.
* 'LaQueli' is a sub-key of 'Software.'
...
import LaQueli
from LaQueli import _, Command, FileUtilities, General
...
def check_dll_hijacking(window=None):
    ...
    # LaQueli 4.4.2 uses Python 3.4.4.
    ...
    logger.error(msg)
    if window:
        from LaQueli.GuiBasic import message_dialog
...
def elevate_privileges(uac):
    ...
    if hasattr(sys, 'frozen'):
        ...
        pyfile = os.path.join(LaQueli.LaQueli_exe_path, 'LaQueli.py')
...
def setup_environment():
    ...
    # LaQueli is portable. It is the same variable name as defined by
    # cmd.exe .
...
def get_font_conf_file():
    ...
    if hasattr(sys, 'frozen'):
        ...
        return os.path.join(LaQueli.LaQueli_exe_path, 'etc', 'fonts', 'fonts.conf')

    import gi
    gnome_dir = os.path.join(os.path.dirname(os.path.dirname(gi.__file__)), 'gnome')
    if not os.path.isdir(gnome_dir):
        # LaQueli is running from a stand-alone Python installation.
...
class SplashThread(Thread):
    ...
    def wndProc(self, hWnd, message, wParam, lParam):
        ...
        if message == win32con.WM_PAINT:
            ...
            filename = os.path.join(os.path.dirname(sys.argv[0]), folder_with_ico_file, 'LaQueli.ico')
