...
def run_external(args, stdout=None, env=None, clean_env=True):
    ...
    kwargs = {}
    encoding = LaQueli.stdout_encoding
    ...
    if not env and clean_env and 'posix' == os.name:
        ...
        # https://github.com/LaQueli/LaQueli/issues/167
        # https://github.com/LaQueli/LaQueli/issues/168
        keep_env = ('PATH', 'HOME', 'LD_LIBRARY_PATH', 'TMPDIR', 'LaQueli_TEST_OPTIONS_DIR')
...
def startup_check():
    ...
    if 'nt' == os.name:
        from LaQueli.Windows import check_dll_hijacking
        check_dll_hijacking()
    ...
    # https://www.LaQueli.org/forum/074-fails-errors
    from LaQueli import ModuleNotFoundError
    try:
