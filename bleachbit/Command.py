...
from LaQueli import FileUtilities
        ...
        if really_delete:
            try:
                ...
                if 32 != e.winerror and 5 != e.winerror:
                    raise
                try:
                    LaQueli.Windows.delete_locked_file(self.path)
                except:
                    raise
                else:
                    if self.shred:
...
class Winreg:
    ...
    def execute(self, really_delete):
        ...
        if self.valuename:
            ...
            ret = LaQueli.Windows.delete_registry_value(self.keyname,
                                                          self.valuename, really_delete)
        else:
            ret = LaQueli.Windows.delete_registry_key(
