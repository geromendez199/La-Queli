...
def install_kde_service_menu_file():
    ...
    service_file_path = data_home_path / "kio" / "servicemenus" / "shred_with_LaQueli.desktop"
...
[Desktop Entry]
Type=Service
Name=Shred With LaQueli
X-KDE-ServiceTypes=KonqPopupMenu/Plugin
MimeType=all/all
Icon=LaQueli
Actions=LaQueliShred
Terminal=true

[Desktop Action LaQueliShred]
Name=Shred With LaQueli
Icon=LaQueli
Exec=kdialog --yesno "This action will shred the following:\n\n$(echo %F | tr ' ' '\n')\n\nContinue?" && sh -c 'LaQueli --shred "$@"; echo Press enter/return to close; read' sh %F
''')
    else:
        try:
