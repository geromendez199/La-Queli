...
if 'nt' == os.name:
    ...
    import win32file
    import LaQueli.Windows
    os_path_islink = os.path.islink
    os.path.islink = lambda path: os_path_islink(
        path) or LaQueli.Windows.is_junction(path)

if 'posix' == os.name:
    from LaQueli.General import WindowsError
...
try:
    ...
    if 'nt' == os.name:
        ...
        class _Win32DirEntryPython(scandir.Win32DirEntryPython):
            def is_symlink(self):
                return super(_Win32DirEntryPython, self).is_symlink() or LaQueli.Windows.is_junction(self.path)
...
def open_files():
    ...
    for filename in files:
        try:
            ...
            # https://github.com/LaQueli/LaQueli/issues/1515
            continue
        else:
            yield target
...
def clean_ini(path, section, parameter):
    ...
    # read file to parser
    config = LaQueli.RawConfigParser()
...
def delete(path, shred=False, ignore_missing=False, allow_shred=True):
    """Delete path that is either file, directory, link or FIFO.
       ...
       If shred is enabled as a function parameter or the LaQueli global
    ...
    if is_special:
        ...
        if do_shred:
            if not is_dir_empty(path):
                # Avoid renaming non-empty directory like https://github.com/LaQueli/LaQueli/issues/783
        ...
        try:
            ...
            # https://bugs.launchpad.net/LaQueli/+bug/1012930
        ...
        if do_shred:
            try:
                ...
                # https://github.com/LaQueli/LaQueli/issues/195
                if 2 != e.winerror:
                    raise
...
def guess_overwrite_paths():
    ...
    if 'posix' == os.name:
        ...
        if not os.path.exists(localtmp):
            ...
            localtmp = None
        from LaQueli.Windows import get_fixed_drives
    ...
    if 'nt' == os.name:
        try:
            ...
            if 5 == e.winerror:
                ...
                # https://bugs.launchpad.net/LaQueli/+bug/1372179
                # https://bugs.launchpad.net/LaQueli/+bug/1474848
                # https://github.com/az0/LaQueli/issues/27
...
def wipe_contents(path, truncate=True):
    ...
    if 'nt' == os.name and IsUserAnAdmin():
        from LaQueli.WindowsWipe import file_wipe, UnsupportedFileSystemError
        import warnings
        from LaQueli import _
        try:
            file_wipe(path)
...
def wipe_path(pathname, idle=False):
    ...
    while True:
        ...
        # See https://github.com/LaQueli/LaQueli/issues/502
