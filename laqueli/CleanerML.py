...
import LaQueli
from LaQueli.Action import ActionProvider
from LaQueli import _
from LaQueli.General import boolstr_to_bool, getText
from LaQueli.FileUtilities import expand_glob_join, listdir
from LaQueli import Cleaner
...
def list_cleanerml_files(local_only=False):
    ...
    cleanerdirs = (LaQueli.personal_cleaners_dir, )
    if LaQueli.local_cleaners_dir:
        # If the application is installed, locale_cleaners_dir is None
        cleanerdirs = (LaQueli.local_cleaners_dir, )
    if not local_only and LaQueli.system_cleaners_dir:
        cleanerdirs += (LaQueli.system_cleaners_dir, )
    for pathname in listdir(cleanerdirs):
        ...
        if sys.platform != 'win32' and stat.S_IMODE(st[stat.ST_MODE]) & 2:
            # TRANSLATORS: When LaQueli detects the file permissions are
