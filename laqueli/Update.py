...
def update_winapp2(url, hash_expected, append_text, cb_success):
    ...
    if os.path.exists(fn):
        ...
        delete_current = True
    # download update
    # FIXME: refactor this to share code with LaQueli.Chaff.download_url_to_fn()
    opener = build_opener()
    ...
    # delete current
    if delete_current:
        from LaQueli.FileUtilities import delete
        delete(fn, True)
    # write file
...
def user_agent():
    ...
    agent = "LaQueli/%s (%s; %s; %s%s)" % (LaQueli.APP_VERSION,
...
def update_dialog(parent, updates):
    ...
    from gi.repository import Gtk
    from LaQueli.GuiBasic import open_url
    dlg = Gtk.Dialog(title=_("Update LaQueli"),
...
def check_updates(check_beta, check_winapp2, append_text, cb_success):
    """Check for updates via the Internet"""
    opener = build_opener()
    socket.setdefaulttimeout(LaQueli.socket_timeout)
    opener.addheaders = [('User-Agent', user_agent())]
    import encodings.idna  # https://github.com/LaQueli/LaQueli/issues/760
    url = LaQueli.update_check_url
