...
def get_system_information():
    ...
    s = "LaQueli version %s" % LaQueli.APP_VERSION

    try:
        # Linux tarball will have a revision but not build_number
        from LaQueli.Revision import revision
        s += '\nGit revision %s' % revision
    except:
        pass
    try:
        from LaQueli.Revision import build_number
    ...
    s += "\nlocal_cleaners_dir = %s" % LaQueli.local_cleaners_dir
    s += "\nlocale_dir = %s" % LaQueli.locale_dir
    s += "\noptions_dir = %s" % LaQueli.options_dir
    s += "\npersonal_cleaners_dir = %s" % LaQueli.personal_cleaners_dir
    s += "\nsystem_cleaners_dir = %s" % LaQueli.system_cleaners_dir
