...
"""
Code that is commonly shared throughout LaQueli
"""
...
from LaQueli import Log
from configparser import RawConfigParser, NoOptionError

APP_VERSION = "4.6.0"
APP_NAME = "LaQueli"
APP_URL = "https://www.LaQueli.org"

socket_timeout = 10

if sys.version_info < (3,0,0):
    print('LaQueli no longer supports Python 2.x.')
    sys.exit(1)
...
# Windows
LaQueli_exe_path = None
if hasattr(sys, 'frozen'):
    # running frozen in py2exe
    LaQueli_exe_path = os.path.dirname(sys.executable)
else:
    # __file__ is absolute path to __init__.py
    LaQueli_exe_path = os.path.dirname(os.path.dirname(__file__))

# license
license_filename = None
license_filenames = ('/usr/share/common-licenses/GPL-3',  # Debian, Ubuntu
                     # Microsoft Windows
                     os.path.join(LaQueli_exe_path, 'COPYING'),
                     '/usr/share/doc/LaQueli-' + APP_VERSION + '/COPYING',  # CentOS, Fedora, RHEL
                     '/usr/share/licenses/LaQueli/COPYING',  # Fedora 21+, RHEL 7+
                     '/usr/share/doc/packages/LaQueli/COPYING',  # OpenSUSE 11.1
                     '/usr/pkg/share/doc/LaQueli/COPYING',  # NetBSD 5
...
options_dir = None
if 'posix' == os.name:
    options_dir = os.path.expanduser("~/.config/LaQueli")
elif 'nt' == os.name:
    os.environ.pop('FONTCONFIG_FILE', None)
    if os.path.exists(os.path.join(LaQueli_exe_path, 'LaQueli.ini')):
        # portable mode
        portable_mode = True
        options_dir = LaQueli_exe_path
    else:
        # installed mode
        options_dir = os.path.expandvars(r"${APPDATA}\LaQueli")

try:
    options_dir = os.environ['LaQueli_TEST_OPTIONS_DIR']
except KeyError:
    pass
        
options_file = os.path.join(options_dir, "LaQueli.ini")
...
if not portable_mode:
    paths = (
        ...
        '../COPYING')
    existing = (
        os.path.exists(os.path.join(LaQueli_exe_path, path))
        for path in paths)
...
# system cleaners
# On Windows in portable mode, the LaQueli_exe_path is equal to
# options_dir, so be careful that system_cleaner_dir is not set to
# personal_cleaners_dir.
if os.path.isdir(os.path.join(LaQueli_exe_path, 'cleaners')) and not portable_mode:
    system_cleaners_dir = os.path.join(LaQueli_exe_path, 'cleaners')
elif sys.platform.startswith('linux') or sys.platform == 'darwin':
    system_cleaners_dir = '/usr/share/LaQueli/cleaners'
elif sys.platform == 'win32':
    system_cleaners_dir = os.path.join(LaQueli_exe_path, 'share\\cleaners\\')
elif sys.platform[:6] == 'netbsd':
    system_cleaners_dir = '/usr/pkg/share/LaQueli/cleaners'
elif sys.platform.startswith('openbsd') or sys.platform.startswith('freebsd'):
    system_cleaners_dir = '/usr/local/share/LaQueli/cleaners'
else:
...
if portable_mode:
    local_cleaners_dir = os.path.join(LaQueli_exe_path, 'cleaners')

# windows10 theme
windows10_theme_path = os.path.normpath(os.path.join(LaQueli_exe_path, 'themes/windows10'))

# application icon
__icons = (
    '/usr/share/pixmaps/LaQueli.png',  # Linux
    '/usr/pkg/share/pixmaps/LaQueli.png',  # NetBSD
    '/usr/local/share/pixmaps/LaQueli.png',  # FreeBSD and OpenBSD
    os.path.normpath(os.path.join(LaQueli_exe_path,
                                  'share\\LaQueli.png')),  # Windows
    # When running from source (i.e., not installed).
    os.path.normpath(os.path.join(LaQueli_exe_path, 'LaQueli.png')),
)
appicon_path = None
...
app_menu_filename = os.path.join(LaQueli_exe_path, 'data', 'app-menu.ui')
...
# system-wide installed locale
    ...
    locale_dir = os.path.join(LaQueli_exe_path, "share\\locale\\")
elif sys.platform[:6] == "netbsd":
...
try:
    ...
    t = gettext.translation('LaQueli', locale_dir)
    _ = t.gettext
except:
    def _(msg):
...
try:
    locale.bindtextdomain('LaQueli', locale_dir)
    ...
    if sys.platform.startswith('win'):
        try:
            ...
            libintl.bindtextdomain(b'LaQueli', locale_dir.encode('utf-8'))
            libintl.bind_textdomain_codeset(b'LaQueli', b'UTF-8')
except:
...
# URLs
#
base_url = "https://update.LaQueli.org"
