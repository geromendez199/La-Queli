...
from LaQueli import GuiBasic
from LaQueli import Cleaner, FileUtilities
from LaQueli import _, APP_NAME, appicon_path, portable_mode, windows10_theme_path
from LaQueli.Options import options

# Now that the configuration is loaded, honor the debug preference there.
from LaQueli.Log import set_root_log_level
set_root_log_level(options.get('debug'))

from LaQueli.GuiPreferences import PreferencesDialog
from LaQueli.Cleaner import backends, register_cleaners
import LaQueli
...
def notify_gi(msg):
    ...
    if Notify.init(APP_NAME):
        notify = Notify.Notification.new('LaQueli', msg, 'LaQueli')
        notify.set_hint("desktop-entry", GLib.Variant('s', 'LaQueli'))
        notify.show()
...
def notify_plyer(msg):
    ...
    """
    from LaQueli import LaQueli_exe_path

    # On Windows 10,  PNG does not work.
    __icon_fns = (
        os.path.normpath(os.path.join(LaQueli_exe_path,
                                      'share\\LaQueli.ico')),
        os.path.normpath(os.path.join(LaQueli_exe_path,
                                      'windows\\LaQueli.ico')))
...
class LaQueli(Gtk.Application):
    ...
    def __init__(self, uac=True, shred_paths=None, auto_exit=False):
        ...
        application_id = '{}{}'.format('org.gnome.LaQueli', application_id_suffix)
        ...
        if os.name == 'nt':
            # clean up nonce files https://github.com/LaQueli/LaQueli/issues/858
            import atexit
            atexit.register(Windows.cleanup_nonce)

        from LaQueli.General import startup_check
        startup_check()
    ...
    def build_app_menu(self):
        ...
        builder = Gtk.Builder()
        builder.add_from_file(LaQueli.app_menu_filename)
    ...
    def cb_help(self, action, param):
        """Callback for help"""
        GuiBasic.open_url(LaQueli.help_contents_url, self._window)

    def cb_make_chaff(self, action, param):
        """Callback to make chaff"""
        from LaQueli.GuiChaff import ChaffDialog
    ...
    def cb_shred_quit(self, action, param):
        ...
        if os.name == 'nt' and portable_mode:
            ...
            # executables
            paths.append(LaQueli.options_file)
            if os.path.isdir(LaQueli.personal_cleaners_dir):
                paths.append(LaQueli.personal_cleaners_dir)
            for f in glob.glob(os.path.join(LaQueli.options_dir, "*.bz2")):
                paths.append(f)
        else:
            paths.append(LaQueli.options_dir)
        ...
        #
        # Rebuild a minimal LaQueli.ini when quitting
    ...
    def get_about_dialog(self):
        dialog = Gtk.AboutDialog(comments=_("Program to clean unnecessary files"),
                                 ...
                                 version=LaQueli.APP_VERSION,
                                 website=LaQueli.APP_URL,
                                 transient_for=self._window)
        try:
            with open(LaQueli.license_filename) as f_license:
    ...
    def quit(self, _action=None, _param=None, init_configuration=False):
        if init_configuration:
            LaQueli.Options.init_configuration()
    ...
    def get_system_information_dialog(self):
        ...
        from LaQueli import SystemInformation
...
class TreeInfoModel:
    ...
    def refresh_rows(self):
        ...
        for key in sorted(backends):
            if not any(backends[key].get_options()):
                ...
                # https://github.com/az0/LaQueli/issues/110
                continue
    ...
    def sort_func(self, model, iter1, iter2, _user_data):
        ...
        last, even for other languages. See https://github.com/LaQueli/LaQueli/issues/441
        """
...
class GUI(Gtk.ApplicationWindow):
    ...
    def __init__(self, auto_exit, *args, **kwargs):
        ...
        bb_logger = logging.getLogger('LaQueli')
        from LaQueli.Log import GtkLoggerHandler
        self.gtklog = GtkLoggerHandler(self.append_text)
        bb_logger.addHandler(self.gtklog)

        # process any delayed logs
        from LaQueli.Log import DelayLog
        ...
        if options.is_corrupt():
            logger.error(
                _('Resetting the configuration file because it is corrupt: %s') % LaQueli.options_file)
            LaQueli.Options.init_configuration()
    ...
    def cb_refresh_operations(self):
        ...
        if not hasattr(self, 'recognized_cleanerml') and not self._auto_exit:
            from LaQueli import RecognizeCleanerML
    ...
    def cb_register_cleaners_done(self):
        ...
        if not self._auto_exit and \
            LaQueli.online_update_notification_enabled and \
        ...
        if os.name == 'nt':
            # BitDefender false positive.  BitDefender didn't mark LaQueli as infected or show
            # anything in its log, but sqlite would fail to import unless BitDefender was in "game mode."
            # http://LaQueli.sourceforge.net/forum/074-fails-errors
            try:
                import sqlite3
        ...
        if os.name == 'posix' and os.path.expanduser('~') == '/root':
            self.append_text(
                _('You are running LaQueli with administrative privileges for cleaning shared parts of the system, and references to the user profile folder will clean only the root account.')+'\n')
        if os.name == 'nt' and options.get('shred'):
            from win32com.shell.shell import IsUserAnAdmin
            if not IsUserAnAdmin():
                self.append_text(
                    _('Run LaQueli with administrator privileges to improve the accuracy of overwriting the contents of files.'))
                self.append_text('\n')

        if 'windowsapps' in sys.executable.lower():
            self.append_text(
                _('There is no official version of LaQueli on the Microsoft Store. Get the genuine version at https://www.LaQueli.org where it is always free of charge.') + '\n', 'error')
