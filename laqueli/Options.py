...
def init_configuration():
    ...
    if not os.path.exists(LaQueli.options_dir):
        General.makedirs(LaQueli.options_dir)
    if os.path.lexists(LaQueli.options_file):
        logger.debug('Deleting configuration: %s ' % LaQueli.options_file)
        os.remove(LaQueli.options_file)
    with open(LaQueli.options_file, 'w', encoding='utf-8-sig') as f_ini:
        f_ini.write('[LaQueli]\n')
        if os.name == 'nt' and LaQueli.portable_mode:
...
class Options:
    ...
    def __init__(self):
        self.purged = False
        self.config = LaQueli.RawConfigParser()
    ...
    def __flush(self):
        ...
        if not os.path.exists(LaQueli.options_dir):
            General.makedirs(LaQueli.options_dir)
        mkfile = not os.path.exists(LaQueli.options_file)
        with open(LaQueli.options_file, 'w', encoding='utf-8-sig') as _file:
            try:
                ...
                if e.errno == ENOSPC:
                    logger.error(
                        _("Disk was full when writing configuration to file %s"), LaQueli.options_file)
                else:
                    raise
        if mkfile and General.sudo_mode():
            General.chownself(LaQueli.options_file)
    ...
    def __purge(self):
        ...
        for option in self.config.options('hashpath'):
            ...
            try:
                ...
                # https://www.LaQueli.org/forum/LaQueli-wont-launch-error-startup
                logger.error(
    ...
    def __set_default(self, key, value):
        ...
        if not self.config.has_option('LaQueli', key):
            self.set(key, value)

    def has_option(self, option, section='LaQueli'):
        """Check if option is set"""
        return self.config.has_option(section, option)

    def get(self, option, section='LaQueli'):
        """Retrieve a general option"""
        if not 'nt' == os.name and 'update_winapp2' == option:
            return False
        if section == 'bleachit' and option == 'debug':
            from LaQueli.Log import is_debugging_enabled_via_cli
    ...
    def is_corrupt(self):
        ...
        for boolean_key in boolean_keys:
            try:
                if self.config.has_option('LaQueli', boolean_key):
                    self.config.getboolean('LaQueli', boolean_key)
            except ValueError:
                return True
        # no int key must raise an exception
        for int_key in int_keys:
            try:
                if self.config.has_option('LaQueli', int_key):
                    self.config.getint('LaQueli', int_key)
            except ValueError:
                return True
        return False
    ...
    def restore(self):
        ...
        try:
            self.config.read(LaQueli.options_file, encoding='utf-8-sig')
        except:
            logger.exception("Error reading application's configuration")
        if not self.config.has_section("LaQueli"):
            self.config.add_section("LaQueli")
        if not self.config.has_section("hashpath"):
            self.config.add_section("hashpath")
        if not self.config.has_section("list/shred_drives"):
            from LaQueli.FileUtilities import guess_overwrite_paths
            try:
        ...
        # LaQueli upgrade or first start ever
        if not self.config.has_option('LaQueli', 'version') or \
                self.get('version') != LaQueli.APP_VERSION:
            self.set('first_start', True)

        # set version
        self.set("version", LaQueli.APP_VERSION)

    def set(self, key, value, section='LaQueli', commit=True):
